    ...
    <Layout>
      <div className="max-w-2xl mx-auto space-y-5 animate-fade-in">
            ...
            <div className="space-y-2">
              {attempts.map(a => (
                <div key={a.attempt_id} className={clsx('flex flex-col sm:flex-row sm:items-center justify-between gap-2 px-3 py-2 rounded border theme-border',
                  a.attempt_id === parseInt(attemptId) && 'result-current-attempt')}>
        ...
        <div>
          ...
          {filtered.length === 0 && <p className="text-center py-8 text-sm theme-muted">No questions in this category</p>}
        </div>
      </div>
    </Layout>
  )
