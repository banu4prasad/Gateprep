...
from app.api.deps import get_current_user, require_admin, get_optional_current_user
from app.core.database import get_db
from app.models.models import (ChecklistProgress, ChecklistSubject,
                               ChecklistTopic)
...
def get_checklist(
    ...
    subjects = (
        db.query(ChecklistSubject)
        .options(joinedload(ChecklistSubject.topics))
        .order_by(ChecklistSubject.order_index)
        .all()
    )
    ...
    if current_user:
        progress_rows = (
            ...
            .filter(ChecklistProgress.user_id == current_user.id)
            .all()
        )
    ...
    for s in subjects:
        ...
        for t in sorted(s.topics, key=lambda x: x.order_index):
            ...
            done_count = sum(
                1 for item in CHECKLIST_ITEMS if completed.get(item, False)
            )
        ...
        result.append(
            {
                ...
                "topics": topics,
                "subject_percentage": (
                    round(subject_done / subject_total * 100) if subject_total else 0
                ),
            }
        )

    return {"subjects": result, "checklist_items": CHECKLIST_LABELS}
...
def update_progress(
    ...
    if payload.item not in CHECKLIST_ITEMS:
        raise HTTPException(
            status_code=400, detail=f"Invalid item. Must be one of: {CHECKLIST_ITEMS}"
        )
    ...
    if not progress:
        progress = ChecklistProgress(
            user_id=current_user.id, topic_id=topic_id, completed_items={}
        )
        db.add(progress)
...
def create_subject(
    ...
    s = ChecklistSubject(
        name=payload.name, order_index=payload.order_index, created_by=current.id
    )
    db.add(s)
    db.commit()
    db.refresh(s)
...
def delete_subject(
    subject_id: int, db: Session = Depends(get_db), _=Depends(require_admin)
):
...
def create_topic(
    ...
    subject = (
        db.query(ChecklistSubject).filter(ChecklistSubject.id == subject_id).first()
    )
    if not subject:
        raise HTTPException(status_code=404, detail="Subject not found")
    t = ChecklistTopic(
        subject_id=subject_id, name=payload.name, order_index=payload.order_index
    )
    db.add(t)
    db.commit()
    db.refresh(t)
...
def delete_topic(
    topic_id: int, db: Session = Depends(get_db), _=Depends(require_admin)
):
